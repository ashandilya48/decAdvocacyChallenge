...
# Load dataset
data = pd.read_csv("carsFixed.csv")

plt.figure(figsize=(8,5))

# Scatter plot
plt.scatter(
    data[data['boss'] == 1]['shopID'],
    data[data['boss'] == 1]['carsFixed'],
    color='seagreen',
    label='Patrick Present',
    alpha=0.6
)
plt.scatter(
    data[data['boss'] == 0]['shopID'],
    data[data['boss'] == 0]['carsFixed'],
    color='yellow',
    label='Not Present',
    alpha=0.6
)

# Compute averages per shop
avg_with_boss = data[data['boss'] == 1].groupby('shopID')['carsFixed'].mean()
avg_without_boss = data[data['boss'] == 0].groupby('shopID')['carsFixed'].mean()

# Plot averages as lines
plt.plot(
    avg_with_boss.index,
    avg_with_boss.values,
    color='seagreen',
    marker='o',
    linestyle='-',
    linewidth=2,
    label='Average with Patrick'
)
plt.plot(
    avg_without_boss.index,
    avg_without_boss.values,
    color='orange',
    marker='o',
    linestyle='--',
    linewidth=2,
    label='Average without Patrick'
)

# Customize x-axis labels
plt.xticks([1, 2, 3, 4, 5], ['Shop 1', 'Shop 2', 'Shop 3', 'Shop 4', 'Shop 5'])

plt.xlabel('Shop')
plt.ylabel('Number of Cars Fixed')
plt.title('Cars Fixed by Shop ID with Patrick\'s Presence and Average Comparison')
plt.legend()
...
sns.barplot(
    ...
    hue="boss",
    palette={
        1: "seagreen",   # Patrick present
        0: "yellow"      # Not present
    }
)
plt.title("Average Productivity by Shop and Patrick's Presence")
plt.xlabel("Shop ID")
plt.ylabel("Average Cars Fixed")
plt.legend(title="Patrick Present (1=Yes, 0=No)")
plt.show()
